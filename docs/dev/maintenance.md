...
**Build-system drift probe.** After the header diff, the script HEAD-probes a handful of named non-header paths (`tools/server/server-http.cpp`, `scripts/ui-assets.cmake`, `tools/ui/ui.{h,cpp}.in`, and the older webui layouts) and reports which layout the target ref uses. Catches drift in non-header files that header diffs miss — exactly the failure mode that bit the b9119 → b9264 bump (the webui asset directory disappeared without any header change) and again at b9318 (`scripts/xxd.cmake` + `tools/ui/ui.h` removed in favour of the `tools/ui/embed.cpp` generator), and again after v0.4.0 (`embed.cpp` removed in favour of `scripts/ui-assets.cmake`). A missing required path fails bump-check with a clear `BUILD-SYSTEM ERROR` instead of surfacing as an opaque `make build` failure later.
