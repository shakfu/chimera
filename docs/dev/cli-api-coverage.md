...
| `--auto-fit` | `--auto-fit` | ✅ | Landed 2026-08-30. Lets sd derive both specs from the model sizes and the per-device memory budgets (`--max-vram`, defaulting to free memory minus a margin), overriding `--backend` / `--params-backend` upstream-side, and possibly splitting a module across GPUs. Fits where an explicit placement would not, but on a single-GPU box it tends to resolve to placing everything on the CPU — prefer an explicit placement when one is known. Maps to `sd_ctx_params_t.auto_fit`. Serve mirror: `--sd-auto-fit`. |
