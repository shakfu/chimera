...
| `-lm` / `--load-mode` | `--load-mode` | ✅ | Landed 2026-07-26 alongside the b10107 bump, which replaced `llama_model_params::use_mmap` / `::use_mlock` with `enum llama_load_mode`. Accepts `auto\|none\|mmap\|mlock\|mmap+mlock\|dio` (CLI11-validated) on gen/chat/embed; `auto`, `mmap+mlock` and `dio` (direct I/O) are reachable only here. Widened 2026-08-30 for llama.cpp `v0.3.0`, which added `LLAMA_LOAD_MODE_AUTO` (-1, and upstream's new default — the loader picks a strategy from the device's capabilities) and split the old combined value into a plain `MLOCK` (lock without mmap) and `MMAP_MLOCK`, moving `DIRECT_IO` from 3 to 4. chimera names the enumerators rather than their values, so only the parser needed widening; `--mlock` keeps mapping to plain `MLOCK`, matching upstream's own deprecated-flag shim. An explicit `--load-mode` wins over `--no-mmap`/`--mlock` — chimera can't reproduce upstream's last-flag-wins ordering because the booleans and the enum live in separate option fields. Upstream's `-lm` short form is dropped: CLI11 rejects multi-character short names. Mapping lives in `chimera_llama_load_mode.h` and is pinned in `tests/external/smoke.cpp`. |
...
| `--tokenizer` | `--tokenizer` | ✅ | Landed 2026-09-23 (sd.cpp master-898). `tokenizer.json` path or `main=`/`clip-l=`/`clip-g=` assignments; required for PiD and Lens. Serve mirror: `--sd-tokenizer`. |
...
| `--disable-prefetch` | `--disable-prefetch` | ✅ | Landed 2026-09-23 (sd.cpp master-898), replacing `--stream-layers`: prefetch is now on by default. Maps to `sd_ctx_params_t.disable_prefetch`. Serve mirror: `--sd-disable-prefetch`. |
| `--disable-segmented-compute` | `--disable-segmented-compute` | ✅ | Landed 2026-09-23. Forces monolithic graphs. Maps to `sd_ctx_params_t.disable_segmented_compute`. Serve mirror: `--sd-disable-segmented-compute`. |
...
| `--backend` | `--backend` | ✅ | Landed 2026-08-30. Per-module *compute* placement spec: a bare target (`cuda0`, `cpu`) or comma-separated assignments (`diffusion=cuda0,te=cpu`). Module keys follow upstream (diffusion/model/unet/dit, te/clip/llm/t5, vae, clip-vision, control-net, photomaker, upscaler, detector). Appended after the `--clip-on-cpu` / `--vae-on-cpu` / `--control-net-cpu` assignments, which sd resolves last-wins, so an explicit spec overrides the coarse flags. Not validated chimera-side — sd parses it and reports its own errors. Serve mirror: `--sd-backend`. |
| `--params-backend` | `--params-backend` | ✅ | Landed 2026-08-30. Per-module *weight residency*, same grammar as `--backend` plus `cpu` / `disk`. Appended after `--offload-to-cpu`. This is the knob that matters when a single GPU cannot hold every module at once: `--params-backend te=cpu` keeps the text encoder's weights in RAM while the diffusion model stays resident. Serve mirror: `--sd-params-backend`. |
| `--auto-fit on\|off` | `--auto-fit` / `--no-auto-fit` | ✅ | Landed 2026-08-30; default flipped to on 2026-09-23 to match sd.cpp master-898. Places weights on the compute GPU, RAM, another GPU or disk by free memory (`--max-vram` caps GPU budgets). A non-empty `--params-backend`, including the one `--offload-to-cpu` composes, disables it. Maps to `sd_ctx_params_t.auto_fit`. Serve mirror: `--sd-auto-fit` / `--sd-no-auto-fit`. |
