...
def tag(status: str) -> str:
    if status == Status.PASS:
        return _ansi("32;1", "PASS")
    if status == Status.FAIL:
        return _ansi("31;1", "FAIL")
    if status == Status.SKIP:
        return _ansi("33;1", "SKIP")
    return status
...
class Recorder:
    ...
    def record(
        self, name: str, status: str, duration: float, detail: str = ""
    ) -> Outcome:
    ...
    def _print(self, o: Outcome) -> None:
        ...
        annot = (
            self._baseline_annotation(o.name, o.duration)
...
def e2e_embed_tests(rec: Recorder, chimera: Path) -> None:
    if EMBED_MODEL is None:
        ...
        rec.skip(
            "embedding cache (bit-identical round trip)",
            f"no embedding model under {MODELS}/",
        )
    ...
    with maybe(rec, "embedding cache (bit-identical round trip)") as t:
        with (
            ...
            with open(out1, "wb") as fh:
                p1 = subprocess.run(
                    cmd, stdout=fh, stderr=subprocess.DEVNULL, timeout=60
                )
    ...
    with maybe(rec, "vector store (rag_test top hit on 'audio file formats')") as t:
        with (
            scratch_file(suffix=".db") as vec_db,
            ...
            rc_i = run_silent(
                [str(chimera), "index", "ingest", "-n", "rag_test", "-f", str(vec_doc)],
                env=env,
                timeout=60,
            )
            if rc_c != 0 or rc_i != 0:
    ...
    with scratch_file(suffix=".db") as hyb_db, scratch_file(suffix=".txt") as hyb_doc:
        hyb_doc.write_text(
        ...
        rc_i = run_silent(
            [str(chimera), "index", "ingest", "-n", "hyb_test", "-f", str(hyb_doc)],
            env=env,
            timeout=60,
        )
...
def e2e_sd_cli_tests(rec: Recorder, chimera: Path) -> None:
    ...
    with maybe(rec, "sd img2img round-trip") as t:
        with scratch_file(suffix=".png") as src, scratch_file(suffix=".png") as dst:
            rc1 = run_silent(
                [
                    str(chimera),
                    "sd",
            ...
            if rc1 != 0 or not src.is_file() or src.stat().st_size == 0:
                ...
                if rc2 != 0 or not dst.is_file() or dst.stat().st_size == 0:
                    t.fail(
                        f"img2img rc={rc2}, size={dst.stat().st_size if dst.exists() else 0}"
                    )
    ...
    if not cn and not cn_image:
        ...
        rec.fail(
            "sd --control-net", 0.0, f"CHIMERA_TEST_CONTROL_IMAGE={cn_image} not found"
        )
    else:
...
def e2e_mtmd_test(rec: Recorder, chimera: Path) -> None:
    ...
    if not (
        MTMD_TEXT_MODEL.is_file() and MTMD_MMPROJ.is_file() and SD_MODEL is not None
    ):
        ...
        with scratch_file(suffix=".png") as img:
            rc_sd = run_silent(
                [
                    ...
                    str(SD_MODEL),
                    "-p",
                    "a single red apple, centered, white background",
                    "-W",
                    "512",
                    "-H",
                    "512",
...
def e2e_chat_cli_tests(rec: Recorder, chimera: Path) -> None:
    if not GEN_MODEL.is_file():
        rec.skip(
            "chat persistent KV cache (recalls 'zephyrine')", f"missing {GEN_MODEL}"
        )
        rec.skip(
            "chat --persist + --list + --search round-trip", f"missing {GEN_MODEL}"
        )
        return
            ...
            with chimera_serve(
                ...
                with maybe(rec, names[1]) as t:
                    if not first_id:
                        ...
                        r = http_post_json(
                            ...
                            headers={"X-Chimera-Chat-Id": first_id},
                            timeout=120,
                        )
                ...
                with maybe(rec, names[2]) as t:
                    r = http_post_json(
                        ...
                        headers={"X-Chimera-Chat-Id": "9999999"},
                        timeout=30,
                    )
                    if r.status != 404:
                ...
                with maybe(rec, names[3]) as t:
                    r = http_post_json(
                        ...
                        headers={"X-Chimera-Chat-Id": "notanint"},
                        timeout=30,
                    )
                    if r.status != 400:
                        t.fail(f"got HTTP {r.status}")
...
def e2e_chats_endpoints_tests(rec: Recorder, chimera: Path) -> None:
    ...
    with scratch_file(suffix=".db") as chat_db, scratch_dir("chimera-pub-") as pub_dir:
        ...
        try:
            with chimera_serve(
                ...
                with maybe(rec, names[2]) as t:
                    ...
                    hits = d.get("hits", [])
                    if not any(
                        "[quibblefrond]" in h.get("snippet", "").lower() for h in hits
                    ):
                        t.fail(f"got: {r.body!r}")
                ...
                with maybe(rec, names[5]) as t:
                    ...
                    if (
                        ...
                        t.fail(
                            f"status={r.status} content-type={ct} body={r.body[:120]!r}"
                        )
        ...
        try:
            with chimera_serve(
                ...
                with maybe(rec, pass2_names[0]) as t:
                    ...
                    if r.status != 200 or not (
                        snap.is_file() and snap.stat().st_size > 0
                    ):
...
def e2e_detect_language_test(rec: Recorder, chimera: Path) -> None:
    ...
    if not (GEN_MODEL.is_file() and WHISPER_MODEL.is_file() and WHISPER_WAV.is_file()):
        rec.skip(
            label, f"needs {GEN_MODEL.name} + {WHISPER_MODEL.name} + {WHISPER_WAV.name}"
        )
        return
    ...
    try:
        with chimera_serve(
            ...
            [
                ...
                str(SD_MODEL),
                "--sd-lora",
                "alpha=/nonexistent/alpha.safetensors",
                "--sd-lora",
                "beta=/nonexistent/beta.safetensors",
            ],
            wait_for_sd=True,
            startup_timeout=240.0,
        ) as srv:
            ...
            with maybe(rec, names[0]) as t:
                ...
                try:
                    ...
                    d = []
                names_present = {
                    item.get("name") for item in d if isinstance(item, dict)
                }
...
def e2e_image_serve_success_paths(rec: Recorder, chimera: Path) -> None:
    ...
    if not lora:
        ...
        try:
            with chimera_serve(
                ...
                pass_label = (
                    "serve loras success (CHIMERA_TEST_LORA fixture, 200 + b64 image)"
                )
                with maybe(rec, pass_label) as t:
                    r = http_post_json(
                        f"{srv.base_url}/v1/images/generations",
                        {
                            "prompt": "a red cube",
    ...
    if not cn and not cn_image:
        ...
        rec.fail(
            label, 0.0, "need BOTH CHIMERA_TEST_CONTROLNET + CHIMERA_TEST_CONTROL_IMAGE"
        )
    elif not Path(cn).is_file():
    ...
    if not pm and not pm_dir:
        rec.skip(
            label, "set CHIMERA_TEST_PHOTOMAKER=<path> + CHIMERA_TEST_PM_ID_DIR=<dir>"
        )
    elif not pm or not pm_dir:
        rec.fail(
            label, 0.0, "need BOTH CHIMERA_TEST_PHOTOMAKER + CHIMERA_TEST_PM_ID_DIR"
        )
    elif not Path(pm).is_file():
            ...
            try:
                with chimera_serve(
                    ...
                    with maybe(rec, pass_label) as t:
                        ...
                        if not (r.status == 200 and _check_b64_image(r.body)):
                            t.fail(
                                f"HTTP {r.status}, first 200 chars: {r.body[:200]!r}"
                            )
            except RuntimeError as e:
                rec.fail(label, 0.0, str(e))
...
def parse_args(argv: list) -> argparse.Namespace:
    ...
    p.add_argument(
        "--smoke", action="store_true", help="Run smoke tests only (skip end-to-end)."
    )
    p.add_argument(
        "--filter", metavar="REGEX", help="Run only tests whose name matches REGEX."
    )
    p.add_argument(
        "--exclude",
        metavar="REGEX",
...
def load_timings_baseline(path: Path) -> dict:
    ...
    if schema not in (TIMINGS_SCHEMA, None):
        ...
        print(
            f"warning: {path}: schema={schema}, expected {TIMINGS_SCHEMA}",
            file=sys.stderr,
        )
    out = {}
...
def main(argv: list) -> int:
    ...
    if chimera is None:
        print(
            f"FAIL: chimera binary not found under {REPO_ROOT}/build/", file=sys.stderr
        )
        print(
            "      searched: build/chimera, build/chimera.exe, "
            "build/{Release,Debug}/chimera[.exe]",
            file=sys.stderr,
        )
    ...
    if args.timings_baseline:
        try:
            ...
            print(
                f"FAIL: --timings-baseline {args.timings_baseline}: {e}",
                file=sys.stderr,
            )
            return 1

    rec = Recorder(
        name_filter=name_filter, name_exclude=name_exclude, baseline=baseline
    )
