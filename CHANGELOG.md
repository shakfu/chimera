...
- **Fetch stable-diffusion.cpp before reading `GGML_MAX_NAME` off it, instead of falling back.** Only reproducible from a clean tree, which is why it survived the incremental builds the fix above was developed against. Builders clone lazily and run in list order — llama.cpp, whisper.cpp, *then* stable-diffusion.cpp — so on a fresh `make reset` build `ggml_max_name()` read SD's `CMakeLists.txt` before that tree existed on disk, silently took `GGML_MAX_NAME_FALLBACK`, and configured llama.cpp and whisper.cpp from a guess. That resolves to the right answer only while the fallback happens to equal SD's current value: the first time upstream moves it and `SDCPP_VERSION` is bumped, a reset build would give llama.cpp/whisper.cpp the stale fallback and SD the new value — reintroducing the exact ABI split this machinery exists to prevent, through the machinery meant to prevent it. `ggml_max_name()` now clones SD on demand when its `CMakeLists.txt` is absent (its own builder clones it moments later regardless, so the only cost is doing it earlier) and both fallback paths now log a warning naming the ABI-split consequence rather than returning quietly.

### Changed

- **Derive `GGML_MAX_NAME` from stable-diffusion.cpp instead of pinning it in two places.** The value belongs to upstream SD, which has moved it before and may again, and a stale copy fails silently at runtime rather than loudly at build time — so re-pinning 160 would only reset the same trap. `StableDiffusionCppBuilder.ggml_max_name()` parses `GGML_MAX_NAME=<n>` out of SD's own `CMakeLists.txt`, and chimera's top-level `CMakeLists.txt` does the same with `file(STRINGS ...)` for its own TUs. `GGML_MAX_NAME_FALLBACK = 160` is a last resort only: the SD tree is fetched on demand so the value can be read from upstream (see above), leaving the fallback to cover an unreadable or unparseable checkout, and it warns when it fires. Configure now reports the resolved value (`-- GGML_MAX_NAME: 160 (from stable-diffusion.cpp)`), which makes a future divergence visible in the build log. Both parsers are inert when `SD_USE_VENDORED_GGML=1`, where each project keeps its own ggml and no agreement is required.
...
- **Verified against the Z-Image Turbo case that exposed it, on CUDA and Vulkan.** `scripts/case/z_turbo.sh` (Z-Image Turbo Q6_K + Qwen3-4B + `--offload-to-cpu --diffusion-fa`, 1024x512) went from a reproducible `Aborted (core dumped)` to exit 0 on both backends, and was run four times on each to rule out the nondeterminism that makes single passes meaningless for heap corruption. The three variants that also aborted before — `--clip-on-cpu --vae-on-cpu`, without `--diffusion-fa`, and at 512x512 — pass on both. `make test` is 68 pass / 0 fail / 7 skip on each, unchanged from before the fix, with the whisper lanes passing on the 64 -> 160 move. Because chimera silences sd.cpp's logging there is no backend banner to read, so GPU residency was confirmed by sampling `nvidia-smi` during a run rather than inferred: CUDA peaked at 6087 MiB / 56% util, Vulkan at 5889 MiB / 77%. The Vulkan leg was built from a clean `make reset`, which is what surfaced the lazy-clone ordering bug above; the CUDA leg was incremental.

- **`GGML_MAX_NAME` agreement is audited per target, and has exactly one legitimate exception.** Every `flags.make` across llama.cpp, whisper.cpp, stable-diffusion.cpp and chimera reports 160 — 87 targets on the Vulkan build. The exception is `vulkan-shaders-gen`, present once per dep, which does *not* carry the define and correctly should not: it is a host tool that compiles GLSL to SPIR-V, and its source includes only standard library headers, no `ggml.h` and no `ggml_tensor`. Any *other* target missing the define is a real mismatch.

- **Not covered.** ROCm/HIP and SYCL were not built. `CMAKE_HIP_FLAGS` is the structurally correct counterpart to `CMAKE_CUDA_FLAGS` — HIP is likewise a separate CMake language — but unlike Vulkan (whose backend is a single `.cpp` reached by `CMAKE_CXX_FLAGS`, so its coverage follows from the C++ path) that cannot be discharged by inspection and needs ROCm hardware to confirm. Separately, note that the 8 GB test card cannot run this model *without* `--offload-to-cpu` on either backend; that path fails cleanly with a 5638 MB allocation error, which is correct behaviour and unrelated to this bug.

## [0.2.11]

### Changed

- **Update llama.cpp version to b10107** (from b9979) and stable-diffusion.cpp to **master-795-87a0177** (from master-775-b5d8120); whisper.cpp stays at **v1.9.1**. Both ranges broke the build, and in both cases the breakage is the same shape: a pair of booleans on a params struct collapsed into a single richer field. The four affected translation units (`chimera_embed.cpp`, `chimera_llama.cpp`, `chimera_pin_check.cpp`, `chimera_sd.cpp`) all failed at compile time — the field-level `static_assert` pin-checks in `chimera_pin_check.cpp` and `chimera_sd.cpp` fired alongside the call sites, which is what they exist for.

### Fixed

- **Adapt to llama.cpp replacing `use_mmap` / `use_mlock` with `llama_load_mode` (b9979 -> b10107).** `llama_model_params::use_mmap` and `::use_mlock` are gone, superseded by `enum llama_load_mode load_mode` with values `NONE` / `MMAP` / `MLOCK` / `DIRECT_IO` (upstream's `--mlock` and `--no-mmap` became deprecated aliases that write the enum). chimera keeps its two booleans on `LlamaCommonOptions` / `EmbedOptions` / `chimera_embed::Config` — they are what `--no-mmap` and `--mlock` bind to — and translates at the llama boundary via a new leaf header, `src/chimera/chimera_llama_load_mode.h`. The mapping mirrors upstream's own deprecated-flag shim in `common/arg.cpp`: `--mlock` selects `LLAMA_LOAD_MODE_MLOCK` (which is mmap + mlock) regardless of the mmap flag, otherwise mmap on/off picks `MMAP` / `NONE`. The header deliberately includes only `llama.h` and `chimera.h` so `chimera_embed.cpp` can share it without pulling in the `common.h` / `mtmd.h` weight of `chimera_llama.h`. The two `use_mmap` / `use_mlock` pin-check `static_assert`s collapse into one on `load_mode`.

- **Adapt to sd.cpp folding the reference-image booleans into `ref_image_args` (master-775 -> master-795).** `sd_img_gen_params_t::increase_ref_index` and `::auto_resize_ref_image` were replaced by `const char * ref_image_args`, a comma- or semicolon-separated `key=value` string that sd resolves on top of a preset chosen from the model architecture (`resolve_ref_image_params`). chimera keeps `--increase-ref-index` and `--no-auto-resize-ref-image` and composes the string in the new `chimera_sd::build_ref_image_args`, translating them exactly as upstream's own deprecated-flag shim does (`ref_index_mode=increase`, `resize_before_vae=0`). Two details matter and are pinned by test: the flags are appended *after* the caller's passthrough string, because sd applies pairs left to right and a repeated key resolves to the last one; and the result is left empty when nothing is set, so the architecture preset's defaults stand rather than being overwritten with chimera-side "defaults". The `increase_ref_index` / `auto_resize_ref_image` pin-checks are replaced by one on `ref_image_args`.

- **Restore the Windows build: compile stable-diffusion.cpp with `/bigobj` (MSVC error C1128).** The sd.cpp bump to master-795 pushed `src/stable-diffusion.cpp` past MSVC's hard cap of 65535 sections per object file, so the Windows CI leg failed in the `deps` stage with `error C1128: number of sections exceeded object file format limit` (followed by a `D8040` from the `/MP` sibling compiles dying alongside the aborted driver). macOS and Linux were unaffected -- GCC and Clang have no equivalent limit -- which is why the break was Windows-only and did not show up locally. sd.cpp does not set the flag in its own `CMakeLists.txt`; upstream works around it in its workflow instead (`.github/workflows/build.yml` configures with `-DCMAKE_CXX_FLAGS='/bigobj'`), so every downstream consumer that configures sd.cpp itself inherits the break. `StableDiffusionCppBuilder.build` now passes the same flag when `PLATFORM == "Windows"`. The flag only widens the object file's section table -- no codegen, ABI, or runtime effect -- and the non-Windows configure path is untouched.

- **Pre-empt the same limit in chimera's own MSVC build.** `/bigobj` was appended to the `WIN32` branch of `CXX_COMPILE_OPTIONS` in the top-level `CMakeLists.txt`, which is the single MSVC flag list for chimera's own translation units: it reaches both `chimera_lib` and the `chimera` executable, each behind a `$<COMPILE_LANGUAGE:CXX>` genex, so it applies to every C++ TU and correctly skips the vendored C sources (`sqlite3.c`, `sqlite-vec.c`, `sha256.c`). The per-modality TUs are the exposure -- `chimera_sd.cpp` pulls in sd.cpp's `ggml_extend.hpp`, `chimera_llama.cpp` pulls in mtmd plus server-context. `tests/external/` deliberately does *not* get the flag: it models an ordinary downstream link against `libchimera*.a`, so if `chimera.hpp` ever needs `/bigobj` in a consumer build, that test should reveal it rather than carry a flag real consumers would not have.

- **Quote multi-token CMake flag values in `manage.py`.** `cmake_config`'s `cmake_flag` helper only quoted values containing a `;` (list separator). A value containing a space -- which `CMAKE_CXX_FLAGS` will have as soon as a second flag joins `/bigobj` -- would have been split by the shell into a broken `-D` argument. It now quotes on whitespace as well.

### Added

- **`--load-mode` (gen / chat / embed): name `llama_load_mode` directly.** Accepts `none | mmap | mlock | dio`, CLI11-validated. Exposes the one mode chimera's booleans cannot reach — `dio` (direct I/O, which silently falls back to a normal read where the platform doesn't support it) — and makes the other three explicit. An explicit `--load-mode` overrides `--no-mmap` / `--mlock`; chimera cannot reproduce upstream's last-flag-wins ordering because the booleans and the enum live in separate option fields, so precedence is fixed rather than positional. Upstream's `-lm` short form is *not* mirrored: CLI11 rejects multi-character short names, and adding it crashes the binary at startup on every subcommand (caught by the `gen --help` smoke test).

- **`--ref-image-args` (`sd`): pass `sd_img_gen_params_t::ref_image_args` through verbatim.** Opens up the reference-image surface that master-795 added and that no chimera flag reached: `preset`, `pass_to_vlm`, `pass_to_dit`, `ref_index_mode` (including `decrease`), `force_ref_timestep_zero`, `resize_before_vae`, `vae_input_max_pixels`, `vlm_resize_mode`, `vlm_min_size`, `vlm_max_size`, `vlm_size`. Not validated chimera-side — sd warns on unknown keys and invalid values at generation time and continues, so a typo degrades rather than aborts, and mirroring the key list here would just add a second place to drift. No serve-side mirror: the image endpoints never exposed the reference-image bundle.

### Testing

- **Pin both translation layers with model-free probes.** `tests/external/smoke.cpp` gains a section that runs before its model-gated inference probe (and returns early on failure, so a real failure isn't folded into ctest's `Skipped` outcome for the missing-model case): a truth table for `chimera_llama_load_mode()` covering the boolean fallbacks, `--load-mode` precedence, `dio`, and `BadInput` on an unknown name; and one for `chimera_sd::build_ref_image_args()` covering each flag alone, both together, passthrough-only, passthrough plus flags, and the colliding-key ordering case. The sd composition was extracted out of `chimera_sd::generate` into a declared pure function specifically so it could be tested without a diffusion model. `scripts/test.py` adds seven CLI-level checks: `--load-mode bogus` rejected on `gen` and `embed`, each of the four valid names reaching model load, and `--ref-image-args` parsing through to model load. `make test` is 18 pass / 0 fail / 51 skip locally (the skips are all model-gated).
...
- **Update llama.cpp version to b9631** (from b9592) and stable-diffusion.cpp to **master-700-c2df4e1** (from master-685-19bdfe2); whisper.cpp stays at v1.8.6. Only sd.cpp required chimera-side code changes (see Fixed). The llama.cpp range was audited by diffing the upstream headers between the two refs directly -- not just by `make bump-check`, which only confirms the *vendored* headers match the *currently-pinned* ref and therefore cannot report what drifted since the previous pin. The diff showed the API headers chimera links against (`llama.h`, `arg.h`, `chat.h`, `sampling.h`, `server-context.h`, `server-http.h`) byte-identical; `common.h` gained one additive field (`mtmd_batch_max_tokens`); `mtmd.h`/`mtmd-helper.h` deprecated `mtmd_encode()` in favor of `mtmd_encode_chunk()` and added an additive batch API -- none of which chimera calls (it uses `mtmd_helper_eval_chunks`); and `server-http.cpp` (which chimera compiles directly) changed 99 lines, all webui asset-serving, handled below. chimera's payload-cap patch still applies (its `set_write_timeout` anchor was untouched).

### Fixed

- **Adapt to sd.cpp's `sd_ctx_params_t` offload rework (master-685 -> master-700).** Upstream removed the per-component offload booleans `vae_decode_only`, `offload_params_to_cpu`, and `keep_{clip,vae,control_net}_on_cpu`, replacing them with backend-assignment spec strings: a global `params_backend` and a per-module compute `backend`. `chimera_sd.cpp::load_model` now translates chimera's existing offload flags into these strings exactly as the upstream example CLI does -- `offload_to_cpu` prepends `*=cpu` to `params_backend`; `keep_clip_on_cpu` / `keep_vae_on_cpu` / `keep_control_net_on_cpu` prepend `te=cpu` / `vae=cpu` / `controlnet=cpu` to `backend` -- with the backing `std::string`s scoped to outlive the `new_sd_ctx()` call that copies them. chimera's public knobs (the `--offload-to-cpu` / `--clip-on-cpu` / `--vae-on-cpu` / `--control-net-cpu` CLI flags, the matching `--sd-*` serve flags, `SdOptions`, and `LoadParams`) are unchanged, so this is a non-breaking translation. `vae_decode_only` has no replacement -- upstream now always builds a full encode+decode VAE (`AutoEncoderKL(..., decode_only=false)`), so the `LoadParams::vae_decode_only` field is retained as an accepted no-op (img2img is unaffected; verified by the `sd img2img round-trip` test). The field-level `static_assert` pin-checks were updated: the five removed fields dropped, `backend` / `params_backend` added.

- **Repair `CHIMERA_WEBUI_EMBED=ON`, broken by the llama.cpp bump.** Two coupled upstream changes in the b9592 -> b9631 range broke the embed path (invisible to `make test`, which only builds the default `OFF`): (1) the host helper `tools/ui/embed.cpp` changed its CLI from a fixed list of `<name> <path>` pairs to a single `<asset_dir>` it embeds recursively (asset names become relative paths); (2) the prebuilt UI is now a SvelteKit/PWA `dist/` tree -- the flat `bundle.{js,css}` are gone, JS/CSS now live content-hashed under `_app/immutable/`, and `server-http.cpp` serves assets dynamically via `llama_ui_get_assets()` instead of fixed `/bundle.*` routes. chimera's integration still assumed the old flat-four-file model, so asset staging silently declined (leaving `src-aux/webui/` empty) and an explicit `ON` build would FATAL at configure. Fixed by adopting upstream's dir-based model: `scripts/manage.py` now stages the entire `dist/` tree (sentinel `index.html`, clear-then-copytree) instead of four named files, and `src/chimera/CMakeLists.txt` passes the staged directory to the generator with a `GLOB_RECURSE CONFIGURE_DEPENDS` dependency list. The `chimera serve` startup banner and its route-catalog comment were updated to drop the stale `/bundle.{js,css}` route names, and `docs/dev/webui.md` documents the new model. Verified end-to-end: an `ON` build serves `GET /` (200 text/html), hashed `_app/immutable/bundle.HASH.js` (200 application/javascript), and 404s unknown assets.
