...
- **Update llama.cpp to v0.4.0** (from v0.3.0); whisper.cpp and stable-diffusion.cpp unchanged. Two upstream signature changes, both caught at compile time. `llm_ffn_exps_block_regex(i)` became `llm_ffn_block_regex(i, ffn_regex)` when upstream generalised the MoE offload helpers to cover dense FFN tensors too (`--n-cpu-ffn`); `--n-cpu-moe` now passes `LLM_FFN_EXPS_REGEX` explicitly. The sibling `llm_add_n_cpu_ffn_overrides()` was not adopted: it parks the pattern strings in a function-local `static std::list` that grows on every call, which is fine for a process that loads one model and wrong for a library whose `load_*` entrypoints can be called repeatedly; chimera already owns the storage in `ModelExtras`. `mtmd_helper_bitmap_init_from_file()` gained a fourth `mtmd_helper_init_opt` argument carrying the video decode parameters; the two call sites pass `mtmd_helper_init_opt_default()`, since `--video-fps`/`--video-timestamp-ms`/`--ffmpeg-dir` reach the decoder through `mtmd_helper_video_init()` on the explicit `--video` path, not through image loading.

  Runtime-visible without a code change: `LLAMA_SESSION_VERSION` 9 -> 10 and `LLAMA_STATE_SEQ_VERSION` 2 -> 3, so slot state written by an earlier chimera (`--slot-save-path`) is rejected on load. Not wrapped: `llama_model_params::lazy_mode` (on-demand tensor reads) and upstream's new `--n-cpu-ffn`.

### Fixed

- **An embedding input longer than the context aborted the process.** `Embedder::embed()` tokenized with `add_special` and handed the result straight to `llama_decode`, but the encoder path asserts `GGML_ASSERT(cparams.n_ubatch >= n_tokens && "encoder requires n_ubatch >= n_tokens")` -- an `abort()` inside llama.cpp, not a return code, so there was nothing downstream to turn into a clean exit. `chimera embed` on any text past the model's context died this way, and so did `chimera index ingest` at its *default* settings: the chunkers clamp `chunk_tokens` to `Embedder::n_ctx()`, which counts content tokens only, so `add_special` then adds the model's `[CLS]`/`[SEP]` on top and a full-width chunk arrives over the line. With `bge-small-en` -- whose 512-token training context is also the default chunk window -- that is the first full chunk of the first file, so ingesting a README with a default-configured collection never worked. `-c/--ctx-size` was no escape; `n_ubatch` is not derived from it. `embed()` now clamps to `min(n_ctx, n_ubatch)`, the same *clamp rather than fail* policy the two guards in `chimera_serve.cpp` already applied on the server paths, and re-seats the trailing separator token after the resize -- `cls` and `last` pooling both read the ends of the sequence, so dropping it would change what those two pool over. The first truncation per `Embedder` warns on stderr; later ones are silent, since ingest drives thousands of chunks through one instance. The `chunk_by_tokens` docs claimed the opposite invariant ("chunks always fit through `embed(text)` without truncation") and have been corrected to describe the special-token headroom rather than promise it.

### Added

- **`scripts/rat.py` ("run artifact tests"), a smoke-test runner for build artifacts.** `make test` tests the tree you just built; nothing tested the binary CI produces, which is the one that gets released. `rat.py` checks it before release: `--bin` takes a bare executable (a downloaded CI artifact or a local build), `--asset` a local archive or URL, and `rat.py install --cuda` resolves a GitHub release (default `/releases/latest`), picks the asset from the backend plus the host -- `--cuda` on Linux is `chimera-<version>-linux-x86_64-cuda.tar.gz` -- and unpacks the binary into `build/rat/`; `rat.py test test-all` then drives that binary's CLI across embeddings, transcription, generation, the vector store and image generation, and `run` is install + test + clean in one command. Two things the suite knows that a hand-run CLI does not: `--gpu-layers` is not optional (chimera's llama-side subcommands default to 0, so a GPU release tested without it would pass every case while measuring nothing but the CPU path), and every case passes `--db`/`--cache-db` explicitly or points `$CHIMERA_DB` into its own output directory, so a smoke run never creates collections in the user's real store. Its entire footprint is one directory, `build/rat/` (the binary, `downloads/` for the archives it came from, `out/` for everything a run produces; `--build-dir`, defaulting to the Makefile's `BUILD_DIR`) -- already gitignored, already what `make clean` sweeps -- except `models/`, which stays at the root shared with `make test` rather than re-downloading tens of GiB after every clean. Ported from cyllama's wheel tester, with the virtualenv machinery dropped -- a static binary has no interpreter to indirect through and no compile-time backend env to set -- and backend detection replaced by parsing `chimera info`'s `built:`/`loaded:` lines, which also surfaces the case worth catching: a GPU build whose runtime brought up only CPU. A `--bin` without its exec bits is `chmod +x`'d before use, because a binary downloaded as a GitHub Actions artifact arrives zipped with its POSIX mode stripped. See [`docs/dev/rat.md`](docs/dev/rat.md).

- **`scripts/case/*.sh` are now cases in that suite.** The twelve one-off shell scripts are gone; each survives as a `rat.py` test target, so they share one model registry, one timeout, one PASS/FAIL summary and one place that knows about `--gpu-layers`. The `z_turbo*.sh` scripts became `test-sd-{1..3}` -- te-on-cpu + vae-tiling, cpu-offload + vae-on-cpu, and cfg-1 + offload + flash-attn -- mirroring the three cases in cyllama's `rwt.py` so the two projects' results compare directly. All three fit an 8 GiB card; the all-on-GPU baseline (~9.4 GiB of weights) and the flash-attn-only variants were dropped, since the baseline OOMs there by construction. `run --fast` now runs `test-sd-3` in place of the old `test-sd-4`. The two `rag-chat-qwen3*.sh` variants became `test-rag-{1,2}` -- which is what found the ingest abort above, since they run at chimera's default chunk window rather than one picked to avoid it.

## [0.2.16]

### Fixed

- **`chunk_by_sentences` never terminated when the chunk window was too small to carry an overlap.** After emitting a packed chunk the loop walks back from the packed range to find the smallest tail worth `overlap_tokens`, then restarts the next pack there. When only one sentence fits at a time -- the next one pushes the window over `chunk_tokens` -- that walk-back always lands on `first`, the sentence the chunk just started with, so the identical singleton was emitted forever while the output vector grew without bound. Two 6-token sentences with `chunk_tokens = 10` and any positive overlap is the minimal reproduction. The guard now requires strict forward progress (`back > first`), dropping the overlap for that one chunk rather than looping; a window that cannot hold two sentences cannot carry an overlap between them either. Reachable remotely: `POST /v1/vector_stores/:name/files` chunks with the *collection's* stored window, so a collection created with a small `chunk_tokens` hung the request thread and exhausted memory.

- **The WAV parser sized an allocation from an unvalidated chunk header.** `parse_wav_stream` read the 32-bit `chunk_size` off the wire and called `pcm_bytes.resize(chunk_size)` before checking that the declared bytes were present, so a ~50-byte multipart upload declaring a 4 GiB `data` chunk forced the allocation -- `std::bad_alloc` or the OOM killer, depending on the box, and on `chimera serve` that takes the whole server down rather than one request. The parser now measures the stream once up front and rejects any chunk claiming more than what remains, and a short read on `data` is an error rather than a silently zero-filled tail. Rejecting rather than clamping is deliberate: a truncated buffer of zeroes transcribes as plausible silence, which is a worse failure than a 4xx.

- **The text-only chat path prepended a second BOS.** The comment above the call says chat templates emit `<|begin_of_text|>` themselves and that `add_special` must therefore be false -- the multimodal path two branches up passes `false` for exactly this reason -- but the text path passed `true`. `chimera tokenize` shows the result directly: a Llama-3 templated prompt came back with `128000 <|begin_of_text|>` twice. Llama-3 reacts by emitting a stray `<|start_header_id|>assistant<|end_header_id|>` and losing track of the user turn. Now `false`, matching both the comment and the mtmd path.

- **RAG ingestion was neither atomic nor safely retryable.** Each chunk was inserted in its own transaction, so a failed embed or insert partway through one request left the leading chunks committed while the caller saw an error -- and since nothing keys on `source_uri`, retrying the same file duplicated everything that had already landed. One file (CLI) / one request (HTTP) is now one transaction. `insert_document` moved from `BEGIN`/`COMMIT` to a savepoint so it nests inside the caller's rather than failing with "cannot start a transaction within a transaction", and the new `chimera_vector_store::Transaction` is an RAII savepoint that rolls back unless `commit()` ran.

- **Row iteration reported a failed query as a short result.** `search_semantic` looped `while (sqlite3_step(...) == SQLITE_ROW)` and returned whatever it had collected, so `SQLITE_BUSY`, `SQLITE_IOERR` or `SQLITE_CORRUPT` mid-iteration surfaced as a valid partial answer -- and in a hybrid search, as the whole semantic leg silently vanishing. The sibling `search_lexical_raw` already checked its terminal status; that pattern is now applied to `search_semantic`, `list` (collections), `list_chats`, `load_messages` and `list_tables`. `find` (collection by name) had the same shape in a different guise: it treated every non-`SQLITE_ROW` status as "no such collection", so a database error was reported as a missing row; it now distinguishes `SQLITE_DONE` from a genuine failure.

- **Persisted media paths were interpolated into JSON rather than serialized.** Interactive chat built `media_json` by concatenating paths between quote characters, so any path containing a quote, a backslash (i.e. every Windows path) or a control character produced a body no JSON reader accepts -- losing the metadata the column exists to carry. It now builds a `nlohmann::json` array and dumps it.

- **`response_format` values outside the documented set were silently accepted.** Anything that was not `text`, `srt`, `vtt` or `verbose_json` fell through to the default `{"text": ...}` body with HTTP 200, so a client that typo'd `srt` had no way to notice. Unsupported values now return 400 naming the accepted set.

### Added

- **Application-level request limits on the audio, image and RAG routes.** The staged cpp-httplib already carries a 256 MB per-request payload cap (`_patch_server_http_payload_cap`), but that is a floor rather than a policy: 256 MB of WAV still decodes to gigabytes of float samples, and the expensive parameters -- image batch count, image dimensions -- are small numbers in a small body that no payload cap can bound. Each route now checks its own bound before committing memory or the model worker: audio upload size (`CHIMERA_MAX_AUDIO_BYTES`, 256 MB), image `n` (`CHIMERA_MAX_IMAGE_BATCH`, 16) and per-side dimensions (`CHIMERA_MAX_IMAGE_DIM`, 4096), and RAG document size (`CHIMERA_MAX_RAG_TEXT_BYTES`, 64 MB) plus chunk count (`CHIMERA_MAX_RAG_CHUNKS`, 100k -- chunk count follows the collection's window, not the request, so the byte cap does not bound it). All are `-D`-overridable at configure time and set far above any legitimate use; the point is that the pathological case fails fast with a 4xx instead of occupying the single-threaded sd or whisper worker indefinitely.

- **A startup warning when the server is reachable off-host with no `--api-key`.** Upstream's `middleware_validate_api_key` gates every route except `/health` and the UI assets -- chimera's own routes included -- so `POST /v1/chimera/shutdown` and the `/v1/chats` history endpoints are protected whenever a key is configured. But it is opt-in, and with no key anyone who can reach the port can stop the process, or read prompts, reasoning and chat contents by walking `/v1/chats/:id` (ids are sequential rowids, authorized by existence alone). That is a defensible default on loopback and a bad surprise off it, so binding to a non-loopback address without a key now says so once on stderr instead of leaving it to the docs.

- **Regression tests for the three defects the suite could not have caught.** An over-declared WAV `data` chunk and an unknown `response_format` are driven through `POST /v1/audio/transcriptions` (the server surviving the first is half the assertion); the chunker case ingests 6-token sentences into a collection with a 10-token window and a positive overlap, where the timeout *is* the assertion -- before the fix that ingest never returned.
