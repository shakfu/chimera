...
- **`scripts/rwt.py`, a smoke-test runner for released binaries.** `make test` tests the tree you just built; nothing tested the artifact users download. `rwt.py install --cuda` resolves a GitHub release (default `/releases/latest`), picks the asset from the backend plus the host -- `--cuda` on Linux is `chimera-<version>-linux-x86_64-cuda.tar.gz` -- and unpacks the binary into `build/rwt/`; `rwt.py test test-all` then drives that binary's CLI across embeddings, transcription, generation, the vector store and image generation, and `run` is install + test + clean in one command. Two things the suite knows that a hand-run CLI does not: `--gpu-layers` is not optional (chimera's llama-side subcommands default to 0, so a GPU release tested without it would pass every case while measuring nothing but the CPU path), and every case passes `--db`/`--cache-db` explicitly or points `$CHIMERA_DB` into its own output directory, so a smoke run never creates collections in the user's real store. Its entire footprint is one directory, `build/rwt/` (the binary, `downloads/` for the archives it came from, `out/` for everything a run produces; `--build-dir`, defaulting to the Makefile's `BUILD_DIR`) -- already gitignored, already what `make clean` sweeps -- except `models/`, which stays at the root shared with `make test` rather than re-downloading tens of GiB after every clean. Ported from cyllama's wheel tester, with the virtualenv machinery dropped -- a static binary has no interpreter to indirect through and no compile-time backend env to set -- and backend detection replaced by parsing `chimera info`'s `built:`/`loaded:` lines, which also surfaces the case worth catching: a GPU build whose runtime brought up only CPU. See [`docs/dev/rwt.md`](docs/dev/rwt.md).

- **`scripts/case/*.sh` are now cases in that suite.** The twelve one-off shell scripts are gone; each survives as a `rwt.py` test target, so they share one model registry, one timeout, one PASS/FAIL summary and one place that knows about `--gpu-layers`. The four distinct `z_turbo*.sh` shapes became `test-sd-{1..4}` (baseline, flash-attn, cfg-1 + flash-attn, cfg-1 + offload + flash-attn), and the two `rag-chat-qwen3*.sh` variants became `test-rag-{1,2}` -- which is what found the ingest abort above, since they run at chimera's default chunk window rather than one picked to avoid it.
