...
- **Option coverage is complete** for `LlamaOptions`, `EmbedOptions`,
  `ServeOptions`, `SdOptions`, and `WhisperOptions`: every field of each struct
  is bound. `tests/test_binding_coverage.py` fails when a `chimera.h` field has
  no `def_rw` in `chimera_ext.cpp`; it reads both files as text and needs no
  built module. `tests/test_module.py` round-trips a subset of each struct.
  `TokenizeOptions` is intentionally not bound -- the `Tokenizer` class takes a
  path + `use_mmap` directly.
